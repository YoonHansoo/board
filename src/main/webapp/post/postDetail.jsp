...
<script>
	$(document).ready(function(){
		...
		$("#updBtn").click(function(){
			 $("#frm").attr("method","get");
			 $("#frm").attr("action","${pageContext.request.contextPath}/postUpd");
				$("#frm").submit();
		...
		$("#delBtn").click(function(){
			...
			if(!confirm('정말로 삭제하시겠습니까?')){
			 ...
			 $("#frm").attr("action","${pageContext.request.contextPath}/postDel");
				$("#frm").submit();
		...
		$("#CmtCreateBtn").click(function(){
			...
			if(!confirm('정말로 등록하시겠습니까?')){
			 ...
			 $("#frm").attr("method","post");
			 $("#frm").attr("action","${pageContext.request.contextPath}/createComment");
				$("#frm").submit();
		});
...
<div class="container-fluid">
	<div class="row">
		...
		<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
			...
			<form id="frm" action="${pageContext.request.contextPath}/postForm"
					...
					<div class="col-sm-7">
						...
						<c:forEach items="${uploadFileList}" var="uploadFileVo">
							<label class="form-control" id="userId"><a
								href="${pageContext.request.contextPath}/download?uploadFileNo=${uploadFileVo.uploadFileNo}">${uploadFileVo.uploadFileName}</a></label>
						</c:forEach>
					</div>
				</div>
				...
				<div class="form-group">
                  ...
                  <label for="userId" class="col-sm-1 control-label">${comment.userId}</label>
					...
					<a href="${pageContext.request.contextPath}/delComments?postNo=${infoMap.postNo}&post_Gn=${infoMap.post_Gn}&boardNo=${infoMap.boardNo}&commentNo=${comment.commentNo}">X</a>
					</c:if>
                   </dd>
                   </c:if>
                  </c:forEach>
